...
if(isset($_GET['activation_code']))
{
	...
	$no_of_row = $statement->rowCount() + 1;
	echo($no_of_row);
	if($no_of_row > 0)
	{
		echo("row count");
		...
		foreach($result as $row)
		{
			if($row['user_email_status'] == 'not verified')
			{
				...
				WHERE user_activation_code = '".$row['user_activation_code']."'
				";
...
<html>
	<head>
		<title>Food Ordering System Email Verification</title>		
